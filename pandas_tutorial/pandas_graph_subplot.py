...
class PandasGraph():
    ...
    def create_line_graph1(self):
        ax231 = plt.subplot(231)
        ax231.plot(self._s1)

    def create_line_graph2(self):
        ax232 = plt.subplot(232)
        ax232.plot(self._s2)
        x1 = self._s2.index

        ax232.set_xticklabels(x1)

    def create_line_graph3(self):
        ax233 = plt.subplot(233)
        ax233.plot(self._s2)
        ax233.grid(True)

    def create_dataframe_to_line_graph1(self):
        ax234 = plt.subplot(234)
        ax234.plot(self._df_rain)
        ax234.grid(True)
        ax234.set_label(self._df_rain.columns)


    def create_dataframe_to_line_graph2(self):
        ax235 = plt.subplot(235)
        ax235.plot(self._df_rain)
        ax235.grid(True)
        ax235.set_xlabel('연도')
        ax235.set_ylabel('강수량')
        ax235.set_title('연간 강수량')
    ...
    def create_pie2(self):
        ...
        fruit_pie = df_fruit.plot.pie(figsize=(5, 5), autopct='%.1f%%', startangle=90, counterclock=False, explode=explode_value, shadow=False, table=True)
